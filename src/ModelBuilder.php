    ...
    static function build( $name_model, $table, $column, $route, $with_timestamp, $with_authstamp, $with_ipstamp, $with_companystamp, $relation )
    {
        ...
        if( !empty($relation) )
        {
            foreach ($relation as $key_relation => $value_relation) {
                ...
                {
                    foreach ($value_relation as $key_belongs_to => $value_belongs_to) {
                        
                        //function belongs to
                        $function = file_get_contents(__DIR__.'/../base/model/function_belongs_to.php', FILE_USE_INCLUDE_PATH);                    
                        if( empty($value_belongs_to['service_name']) )
                        {
                            $function = str_replace('{{belongs_to_name_model}}',ucwords(camel_case($value_belongs_to['name'])),$function);
                        }
                        else 
                        {
                            $function = str_replace('{{belongs_to_name_model}}',ucwords(camel_case($value_belongs_to['service_name'])),$function);
                        }
                        $function = str_replace('{{belongs_to_name}}',ucwords($value_belongs_to['name']),$function);
                        $function = str_replace('{{column_belongs_to_foreign_key}}',$value_belongs_to['foreign_key'],$function);                    
                        ...
                        $column_belongs_to = self::generateColumnRelation($value_belongs_to['select_column']);
                        
                        $belongs_to_query = str_replace('{{column_belongs_to}}',"".$column_belongs_to,$belongs_to_query);
                        $belongs_to_query = str_replace('{{table_belongs_to}}',$value_belongs_to['table'],$belongs_to_query);
                        $belongs_to_query = str_replace('{{column_belongs_to_foreign_key}}',$value_belongs_to['foreign_key'],$belongs_to_query);                    
                        $belongs_to_query = str_replace('{{belongs_to_name}}',$value_belongs_to['name'],$belongs_to_query);                                                

                        if( !empty($value_belongs_to['custom_join']) )
                        {
                            $belongs_to_query = str_replace('-- end list belongs to join option',$value_belongs_to['custom_join']."\r\n\t\t\t\t\t\t".'-- end list belongs to join option',$belongs_to_query);
                        }
                        if( !empty($value_belongs_to['custom_option']) )
                        {
                            $belongs_to_query = str_replace('-- end list belongs to query option',$value_belongs_to['custom_option']."\r\n\t\t\t\t\t\t".'-- end list belongs to query option',$belongs_to_query);
                        }
                ...
                {
                    foreach ($value_relation as $key_has_one => $value_has_one) {
                        
                        //function has one
                        $function = file_get_contents(__DIR__.'/../base/model/function_has_one.php', FILE_USE_INCLUDE_PATH);                    
                        $function = str_replace('{{has_one_name}}',ucwords($value_has_one['name']),$function);
                        $function = str_replace('{{column_has_one_foreign_key}}',$value_has_one['foreign_key'],$function);                    
                        $base_model = str_replace('// end list relation function',$function,$base_model);

                        // column has one
                        $has_one_query = file_get_contents(__DIR__.'/../base/model/query_column_has_one.php', FILE_USE_INCLUDE_PATH);
                        $column_has_one = self::generateColumnRelation($value_has_one['select_column']);
                        
                        $has_one_query = str_replace('{{column_has_one}}',"".$column_has_one,$has_one_query);
                        $has_one_query = str_replace('{{table_has_one}}',$value_has_one['table'],$has_one_query);
                        $has_one_query = str_replace('{{column_has_one_foreign_key}}',$value_has_one['foreign_key'],$has_one_query);                    
                        $has_one_query = str_replace('{{has_one_name}}',$value_has_one['name'],$has_one_query);
                        if( !empty($value_has_one['custom_join']) )
                        {
                            $has_one_query = str_replace('-- end list has one join option',$value_has_one['custom_join']."\r\n\t\t\t\t\t\t".'-- end list has one join option',$has_one_query);
                        }
                        if( !empty($value_has_one['custom_option']) )
                        {
                            $has_one_query = str_replace('-- end list has one query option',$value_has_one['custom_option']."\r\n\t\t\t\t\t\t".'-- end list has one query option',$has_one_query);
                        }
                ...
                {
                    foreach ($value_relation as $key_has_many => $value_has_many) {

                        // function has many                    
                        $function = file_get_contents(__DIR__.'/../base/model/function_has_many.php', FILE_USE_INCLUDE_PATH);
                        $function = str_replace('{{has_many_name}}',ucwords($value_has_many['name']),$function);
                        $function = str_replace('{{column_has_many_foreign_key}}',$value_has_many['foreign_key'],$function);                    
                        $base_model = str_replace('// end list relation function',$function,$base_model);

                        // column has many
                        $has_many_query = file_get_contents(__DIR__.'/../base/model/query_column_has_many.php', FILE_USE_INCLUDE_PATH);
                        $column_has_many = self::generateColumnRelation($value_has_many['select_column']);
                                   
                        $has_many_query = str_replace('{{column_has_many}}',"".$column_has_many,$has_many_query);
                        $has_many_query = str_replace('{{table_has_many}}',$value_has_many['table'],$has_many_query);
                        $has_many_query = str_replace('{{column_has_many_foreign_key}}',$value_has_many['foreign_key'],$has_many_query);                    
                        $has_many_query = str_replace('{{has_many_name}}',$value_has_many['name'],$has_many_query);
                        if( !empty($value_has_many['custom_join']) )
                        {
                            $has_many_query = str_replace('-- end list has many join option',$value_has_many['custom_join']."\r\n\t\t\t\t\t\t".'-- end list has many join option',$has_many_query);
                        }
                        if( !empty($value_has_many['custom_option']) )
                        {
                            $has_many_query = str_replace('-- end list has many query option',$value_has_many['custom_option']."\r\n\t\t\t\t\t\t".'-- end list has many query option',$has_many_query);
                        }
                ...
                {
                    foreach ($value_relation as $key_belongs_to_many => $value_belongs_to_many) {

                        // function belongs to many
                        $function = file_get_contents(__DIR__.'/../base/model/function_belongs_to_many.php', FILE_USE_INCLUDE_PATH);
                        $function = str_replace('{{belongs_to_many_name}}',UCWORDS($value_belongs_to_many['name']),$function);
                        $function = str_replace('{{belongs_to_many_table}}',$value_belongs_to_many['table'],$function);
                        $function = str_replace('{{column_belongs_to_many_foreign_key_model}}',$value_belongs_to_many['foreign_key_model'],$function);
                        $function = str_replace('{{column_belongs_to_many_foreign_key_joining_model}}',$value_belongs_to_many['foreign_key_joining_model'],$function);                    
                        $base_model = str_replace('// end list relation function',$function,$base_model);
                        ...
                        $column_belongs_to_many = self::generateColumnRelation($value_belongs_to_many['select_column']);
                        
                        $belongs_to_many_query = str_replace('{{column_belongs_to_many}}',"".$column_belongs_to_many,$belongs_to_many_query);
                        $belongs_to_many_query = str_replace('{{belongs_to_many_intermediate_table}}',$value_belongs_to_many['intermediate_table'],$belongs_to_many_query);
                        $belongs_to_many_query = str_replace('{{belongs_to_many_table}}',$value_belongs_to_many['table'],$belongs_to_many_query);                    
                        $belongs_to_many_query = str_replace('{{foreign_key_model}}',$value_belongs_to_many['foreign_key_model'],$belongs_to_many_query);
                        $belongs_to_many_query = str_replace('{{foreign_key_joining_model}}',$value_belongs_to_many['foreign_key_joining_model'],$belongs_to_many_query);
                        $belongs_to_many_query = str_replace('{{belongs_to_many_name}}',$value_belongs_to_many['name'],$belongs_to_many_query);
                        if( !empty($value_belongs_to_many['custom_join']) )
                        {
                            $belongs_to_many_query = str_replace('-- end list belongs to many join option',$value_belongs_to_many['custom_join']."\r\n\t\t\t\t\t\t".'-- end list belongs to many join option',$belongs_to_many_query);
                        }
                        if( !empty($value_belongs_to_many['custom_option']) )
                        {
                            $belongs_to_many_query = str_replace('-- end list belongs to many query option',$value_belongs_to_many['custom_option']."\r\n\t\t\t\t\t\t".'-- end list belongs to many query option',$belongs_to_many_query);
                        }
                        
                        $cols_table_model .= $belongs_to_many_query."\r\n";

                        // start build migration for intermediate table                    
                        $intermediate_name = $name_model.'_'.$value_belongs_to_many['name'];
                        $intermediate_table_name = $value_belongs_to_many['intermediate_table'];
                        
                        // set default column
                        $value_belongs_to_many['column'] = [
                        [
                            'name'  =>  $value_belongs_to_many['foreign_key_model'],
                            'type'  =>  'integer',
                        ],
                        [
                            'name'  =>  $value_belongs_to_many['foreign_key_joining_model'],
                            'type'  =>  'integer',
                        ],  
                        ];

                        $value_belongs_to_many = columnBuilder::build($value_belongs_to_many,'column');
                        
                        if( !empty($value_belongs_to_many['column_add_on']) )
                        {
                            $value_belongs_to_many['column'] = array_merge($value_belongs_to_many['column_add_on'],$value_belongs_to_many['column']);
                        }

                        MigrationBuilder::build($intermediate_name,$intermediate_table_name,$value_belongs_to_many['column']);
