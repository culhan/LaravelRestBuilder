...
class ModelBuilder
{
     ...
     * @return void
     */
    static function build( $name_model, $table, $key, $column, $column_function = [], $route, $with_timestamp, $with_authstamp, $with_ipstamp, $with_companystamp, $custom_filter, $relation, $hidden )
    {
        ...
        if( !empty($key) ) {
            $option_key = file_get_contents(__DIR__.'/../base/model/option_key.php', FILE_USE_INCLUDE_PATH);
        ...
        if( !empty($custom_filter) ) {
            $option_custom_filter = file_get_contents(__DIR__.'/../base/model/option_query_custom_filter.php', FILE_USE_INCLUDE_PATH);
        ...
        if( !empty($relation) )
        {
            foreach ($relation as $key_relation => $value_relation) {
                ...
                {                    
                    ...
                    $belongs_to_query = str_replace('{{column_belongs_to_relation_key}}',(!empty($value_relation['relation_key']) ? $value_relation['relation_key']:'id' ),$belongs_to_query);
                ...
                {                    
                    ...
                    $has_one_query = str_replace('{{column_has_one_relation_key}}',(!empty($value_relation['relation_key']) ? $value_relation['relation_key']:'id' ),$has_one_query);                    
