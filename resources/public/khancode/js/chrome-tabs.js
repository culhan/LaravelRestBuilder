((window, factory) => {
  ...
  class ChromeTabs {
    ...
    removeTab(tabEl,status = false) {         
      if ($(tabEl).data('changed') && !status) {
        return
      }
      if (tabEl === this.activeTabEl) { 

        //   console.log('js:' + $(tabEl.nextElementSibling).attr('class'))

        if (tabEl.nextElementSibling && !$(tabEl.nextElementSibling).hasClass('addtab')) {
